// To run from ../src
// g++ -fopenmp -O2 -o exe src/serialWithOpenMP.cpp
// ./exe
...
#include <iomanip>
#include <omp.h>
#include <functional>  // Added missing header for std::function
using namespace std;
...
// Parallelize the edge relaxation (note: may not always help due to overhead)
#pragma omp parallel for
            for (size_t i = 0; i < outgoing_edges[u].size(); i++)
            {
                const Edge &edge = outgoing_edges[u][i];
                int v = edge.target;
                int weight = edge.weight;

// Need critical section for distance updates
#pragma omp critical
                {
...
// Parallelize checking of outgoing edges
#pragma omp parallel for
            for (size_t i = 0; i < outgoing_edges[u].size(); i++)
            {
                const Edge &edge = outgoing_edges[u][i];
...
#pragma omp critical
    ...
    {
// Reset distances for affected vertices (parallel)
#pragma omp parallel for
        for (size_t i = 0; i < affected_vertices.size(); i++)
        {
            int v = affected_vertices[i];
            if (v != source_vertex)
            {
...
// Parallelize checking incoming edges
#pragma omp parallel for
        for (size_t i = 0; i < affected_vertices.size(); i++)
        {
            int v = affected_vertices[i];
...
#pragma omp critical
        ...
        // The main Dijkstra loop remains sequential
        while (!pq.empty())
        {
            int u = pq.top().vertex;
            int dist_u = pq.top().distance;
...
int main()
{
    cout << "\n\n----------------------------------------\n";
    cout << "----------------------------------------\n";
    cout << "Serial with Open MP Implementation \n";
    cout << "----------------------------------------\n";
    cout << "----------------------------------------\n";
    int source_vertex = 0;
    ...
    cout << "Graph loading time: " << fixed << setprecision(6) << (end_time - start_time) << " seconds\n";
    ...
    end_time = omp_get_wtime();
    cout << "Initial SSSP computation time: " << fixed << setprecision(6) << (end_time - start_time) << " seconds\n";
    ...
    changes_file.close();
    end_time = omp_get_wtime();
    cout << "Changes incoporated in time: " << fixed << setprecision(6) << (end_time - start_time) << " seconds\n";
